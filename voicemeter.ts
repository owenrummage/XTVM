...
import { ControlType } from "../src/right";
...
function setBusSends(vm: Voicemeeter, controller: XTouchController) {
    for (let i = 0; i < 8; i++) {
        const vmBusName = VoicemeeterChannelNames[selectedBus] as keyof typeof VoicemeeterChannelNames;
...
async function run() {
    ...
    controller.on("fade", async (key) => {
        ...
        console.log(dbValue);
        if (key.channel === 9) { // Use the 9th output fader for the selected bus
            ...
            switch (e.action) {
                case "select": {
                    if (functionMode === "BUSSES") {
                        ...
                        const vmBusName = VoicemeeterChannelNames[bus] as keyof typeof VoicemeeterChannelNames;

                        const curState = Math.floor(vm.parameters.Strip(vmInChannel)[vmBusName].get());

                        let newState = 0;
                        if (curState === 0) newState = 1; // If 0 then 1 and if 1 stay 0

                        vm.parameters.Strip(vmInChannel)[vmBusName].set(newState);
                        controller.channel(e.channel).setButton("SEL", (newState === 1) ? "SOLID" : "OFF")
                    }
                    break;
                }
                case "mute": {
                    if (functionMode === "BUSSES") {
                        ...
                        const vmInChannel = e.channel - 1;
                        const isMutedNumber = Math.floor(vm.parameters.Strip(vmInChannel).Mute.get())
                        const isMuted = (isMutedNumber === 1) ? true : false
                        console.log(e.channel, "Muted:", isMuted)
                        if (isMuted === false) { // Was not muted, mute
                        ...
                        } else { // Was muted, unmute and wait for keyUp (state = true and time set) to test for PTT
            ...
            switch (e.action) {
                case "mute": {
                    ...
                    if (functionMode === "BUSSES") {
                        ...
                        if (isMuted === false && pttStates[e.channel].state === true) { // Was muted on keyDown and now is unmuted, state is true so we should check for ptt
                            const newTime = dayjs()
                            const oldTime = pttStates[e.channel].time
                            const diffTime = newTime.diff(oldTime, 'millisecond')
                            if (diffTime > 750) { // The button was held for more than 3/4th of a second
                                // PTT -> Remute
    ...
    while (true) {
        ...
        for (let channel = 0; channel < 8; channel++) {
            ...
            const mappedValue = Math.round(Math.max(0, Math.min(8, Math.pow(meterState, 0.5) * 8)));
